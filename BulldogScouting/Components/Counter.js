...
const Counter = ({ store, variable, disabled }) => {
    const count = Stores[store](state => state[variable]);
    const set = Stores[store](state => state.set);

    const handleIncrement = () => {
        if (!disabled) {
            set({ [variable]: [...count , new Date().toISOString()] });
            console.log(count);
        }
    };

    const handleDecrement = () => {
        if (!disabled && count.length > 0) {
            set({ [variable]: count.slice(0, -1)});
        }
    };
    ...
    return (
        <HStack>
            ...
            <Text style={{ width: 75, textAlign: 'center', fontSize: 24 }}>  {count.length}  </Text>
